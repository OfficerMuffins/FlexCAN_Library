...
// -------------------------------------------------------------
// Finds an available TX buffer by searching through 64 bits (0x10 * 32)
// of memory starting at flexcan base. A buffer is available if the 0x8 << 24
// bit is 1: if the 27th bit is set.
// TODO: override checking of the buffer
// -------------------------------------------------------------

// set this macro to 1 in order to override the MB check
#define OVERRIDE_BUFF 0
int FlexCAN::write(const CAN_message_t &msg)
{
  ...
  buffer = -1;
  for ( int index = txb; ; ) {
    Serial.print("Trying buffer"); Serial.println(index);
    // checks if buffer is inactive
...
#endif
  Serial.print("Using buffer"); Serial.println(buffer);
  // transmit the frame
  ...
  if(msg.ext) {
    ...
    // sets the buffer to be inactive
    FLEXCANb_MBn_CS(flexcanBase, buffer) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_ONCE)
...
/*
 * clears all MBs starting at rxb
 */
void FlexCAN::clear_all_MBs()
{
  for (int buff = rxb; buff < 16; buff++) {
    FLEXCANb_MBn_CS(flexcanBase, buff) = FLEXCAN_MB_CS_CODE(FLEXCAN_MB_CODE_TX_INACTIVE);
  }
}
